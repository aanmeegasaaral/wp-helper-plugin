...
class Post_Creator extends \VSP\Base {
	public function __construct() {
		wponion_admin_page( array(
			'menu_title' => __( 'Narayaneeyam Creator', 'aanmeegasaaral' ),
			'page_title' => __( 'Narayaneeyam Creator', 'aanmeegasaaral' ),
...
HTML;
		...
		$select            = $builder->select( 'narayaneeyam_lyrics', __( 'Narayaneeyam Lyrics', 'aanmeegasaaral' ) )->query_args( array(
			'post_type'      => 'narayaneeyam_lyrics',
			'posts_per_page' => 10,
		) )->select_framework( 'select2' )->ajax( true )->style( 'width:25%;' );
		$select['options'] = 'post';
		$builder->select( 'narayaneeyam_part', __( 'Narayaneeyam Part', 'aanmeegasaaral' ) )
			->option( '', __( 'All', 'aanmeegasaaral' ) )
			->option( 'part1', __( 'Part 1', 'aanmeegasaaral' ) )
			->option( 'part2', __( 'Part 2', 'aanmeegasaaral' ) )
